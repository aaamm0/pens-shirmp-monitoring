        ...
        <div class="card info-card d-flex flex-column align-items-center justify-content-center">
            <div class="card-body text-center">
                ...
                <div class="chart-container d-flex justify-content-center align-items-center" style="width: 350px; height: 355px;">
                    ...
                    <h3 id="doValue" class="position-absolute text-secondary" style="font-size: 24px; font-weight: bold;"></h3>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        let doChart = new ApexCharts(document.querySelector("#dissolvedOxygenChart"), {
        ...
        function updateDOChart() {
            fetch('get-card-data.php')
                .then(response => response.json())
                .then(data => {
                    let doValue = parseFloat(data.dissolveOxygen).toFixed(2);
                    let normalizedValue = Math.max(minDO, Math.min(doValue, maxDO));
                    let seriesValue = ((normalizedValue - minDO) / (maxDO - minDO)) * 100;
                    
                    doChart.updateSeries([seriesValue]);
                    document.getElementById("doValue").innerText = doValue + " mg/L";
                })
                .catch(error => console.error('Error fetching data:', error));
        }
