    ...
    <div class="col-lg-4 col-md-12">
        <div class="card info-card d-flex flex-column align-items-center justify-content-center">
            <div class="card-body text-center">
                <h5 class="card-title">Salinitas</h5>
                <div class="chart-container d-flex justify-content-center align-items-center" style="width: 350px; height: 355px;">
                    ...
                    <h3 id="salValue" class="position-absolute text-secondary" style="font-size: 24px; font-weight: bold;"></h3>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        let doChart = new ApexCharts(document.querySelector("#salinityChart"), {
        ...
        function updateDOChart() {
            fetch('get-card-data.php')
                .then(response => response.json())
                .then(data => {
                    let salValue = parseFloat(data.salinity).toFixed(2);
                    let normalizedValue = Math.max(minDO, Math.min(salValue, maxDO));
                    let seriesValue = ((normalizedValue - minDO) / (maxDO - minDO)) * 100;

                    doChart.updateSeries([seriesValue]);
                    document.getElementById("salValue").innerText = salValue + " ppt";
                })
                .catch(error => console.error('Error fetching data:', error));
        }
